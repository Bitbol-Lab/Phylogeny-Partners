                ...
                for index_spin_1 in range(n_states):
                    for index_spin_2 in range(n_states):
                        if site_i != site_j:
                            ...
                            Fij[site_i,index_spin_1,site_j,index_spin_2] = (index_spin_1 == index_spin_2)*1.0*(pseudocount/n_states) + (1-pseudocount)*Fij[site_i,index_spin_1,site_j,index_spin_2]
    return np.asarray(Fij), np.asarray(weight)
...
def image_plot_freq(double[:,:,:,:] freqs1, double[:,:,:,:] freqs2, int val=1000, double xmin=0, double xmax=1):
    cdef:  
    ...
    for i in prange(freqs1.shape[0] - 2, nogil=True):
        for j in range(freqs1.shape[1]):
            for k in range(i+1, freqs1.shape[2]):
...
def image_plot_freq_simple(double[::1] freqs1, double[::1] freqs2, int val=1000, double xmin=0, double xmax=1):
    cdef:  
        int[:,::1] ima = np.zeros((val,val), dtype=np.intc)
        int i,pos1,pos2
        
    for i in prange(freqs1.shape[0], nogil=True):
                    pos1 = freq_to_pos(freqs1[i], val, xmin, xmax)
                    pos2 = freq_to_pos(freqs2[i], val, xmin, xmax)
                    ima[pos1, pos2] += 1
    return np.asarray(ima)


cdef int freq_to_pos(double freq, int val, double xmin, double xmax) nogil:
    return int((freq-xmin)*(val-1)/(xmax-xmin))